...
public class BoardTile : MonoBehaviour
{
    ...
    public virtual void OnLanded(Player player)
    {
        ...
        switch (tileType)
        {
            case TileType.Start:
                ...
                if (GameManager.Instance != null)
                {
                    ...
                    Debug.Log($"{player.playerName} 到达起点工资: {salary} 元");
            ...
            case TileType.Buildable:
                // 可建造地块
                Debug.Log($"{player.playerName} 来到可建造地块: {tileName}");
                if (ownerPlayer == null)
                {
                    // 显示建筑选择UI
                    if (UIManager.Instance != null)
            ...
            case TileType.BuildingSite:
                // 建筑地块
                Debug.Log($"{player.playerName} 来到建筑地块: {tileName}");
        ...
        Debug.Log($"强制触发Debug");
        Debug.Log($"强制触发关联收入检查，tileType: {tileType}");
        // === 强制触发关联收入检查 ===
        if (enableLinkedIncome && linkedBuildingTiles != null && linkedBuildingTiles.Count > 0)
        {
            Debug.Log($"该地块 {tileName} 有 {linkedBuildingTiles.Count} 个关联建筑");
    ...
    private void TriggerLinkedBuildingIncome(Player player)
    {
        ...
        {
            Debug.LogWarning($"  [失败] 该地块 'enableLinkedIncome' 为 false，直接跳过");
            return;
        }
        ...
        for (int i = 0; i < linkedBuildingTiles.Count; i++)
        {
            ...
            Debug.Log($"  -- 检查建筑 [{i + 1}/{linkedBuildingTiles.Count}]: {buildingTile?.tileName ?? "NULL"} --");

            if (buildingTile == null)
            {
                Debug.LogWarning($"    [警告] 列表中的第 {i + 1} 个关联建筑为 null");
                continue;
            }
            ...
            if (CanGenerateIncome(buildingTile, currentTime))
            {
                ...
                {
                    Debug.Log($"    [信息] 建筑拥有者: {buildingTile.ownerPlayer.playerName}， 当前玩家: {player.playerName}");

                    if (buildingTile.ownerPlayer == player)
                    {
                        Debug.Log($"    [通过] 建筑拥有者是当前玩家");

                        // 6. 检查建筑数据
                        if (buildingTile.currentBuildingData != null)
                        {
                            Debug.Log($"    [信息] 建筑名称: {buildingTile.currentBuildingData.buildingName}, 功能类型: {buildingTile.currentBuildingData.functionType}");
                                ...
                                buildingTile.currentBuildingData.functionType == BuildingData.BuildingFunctionType.Mixed)
                            {
                                ...
                                Debug.Log($"    [信息] 建筑等级: {buildingTile.buildingLevel}, 收入金额: {incomeAmount}");
                                ...
                                if (incomeAmount > 0)
                                {
                                    ...
                                    Debug.Log($"    [成功] 为玩家 {player.playerName} 从建筑 {buildingTile.currentBuildingData.buildingName} 获得收入: {incomeAmount} 元");
                                ...
                                {
                                    Debug.LogWarning($"    [失败] 该建筑的收入为 0，跳过");
                                }
                            }
                            else
                            {
                                Debug.LogWarning($"    [失败] 该建筑功能类型不是 Income 或 Mixed，当前类型: {buildingTile.currentBuildingData.functionType}");
                            }
                        }
                        else
                        {
                            Debug.LogWarning($"    [失败] 该建筑地块没有建筑数据 (currentBuildingData 为 null)");
                        }
                    }
                    else
                    {
                        Debug.LogWarning($"    [失败] 该建筑拥有者不是当前玩家");
                    }
                }
                else
                {
                    Debug.LogWarning($"    [失败] 该建筑地块没有拥有者 (ownerPlayer 为 null)");
                }
            }
            else
            {
        ...
        {
            Debug.Log($"=== [调试] 关联收入检查完成，为玩家 {player.playerName} 获得: {totalIncome} 元 ===");
        ...
        {
            Debug.LogWarning($"=== [调试] 关联收入检查完成，未产生任何收入。 ===");
        }
    ...
    private bool CanGenerateIncome(BoardTile buildingTile, float currentTime)
    {
        if (!lastIncomeTime.ContainsKey(buildingTile))
        {
            Debug.Log($"    [信息] 建筑 {buildingTile.tileName} 还从未产生过收入，可以产生");
            return true;
        }
        ...
        Debug.Log($"    [信息] 建筑 {buildingTile.tileName} 上次收入距今 {timeSinceLastIncome:F1} 秒，需要间隔 {incomeInterval} 秒，可以产生: {canGenerate}");
        return canGenerate;
    }
