...
public class EventEffectHandler : MonoBehaviour
{
    ...
    public void ProcessOption(Player player, EventData eventData, int optionIndex)
    {
        ...
        {
            Debug.LogError($"? ?????????完: {optionIndex}, ???????: {eventData.options.Length}");
        }
    }
    ...
    {
        Debug.Log($"? ApplyEventEffects ?????孝?optionIndex={optionIndex}");
        ...
        Debug.Log($"? 完??????: cost={costToPay}, reward={rewardToGive}, boost={incomeBoostToApply}, duration={buffDurationToSet}");
    ...
    private void ShowResultToast(Player player, EventData eventData, int optionIndex)
    {
        ...
        {
            message += $"??芍???????({eventData.buffDurationRounds}???????+{eventData.incomeBoost*100:0}%)";
        }
