...
public class UIManager : MonoBehaviour
{
    ...
    public void OnTileClickedForPlacement(BoardTile tile)
    {
        ...
        if (PurchaseAndPlaceBuilding(tile, selectedBuildingData, currentBuildingPlayer))
        {
            ...
            // 不再弹 Toast "建造成功"：音效 + 建筑出现在地图上 + 面板重新弹出已提供充分反馈，
            // 且避免 Toast 与重新显示的建筑选择面板互相遮挡。
        }
    }
    ...
    private void EnqueueToast(string message, float duration, bool isAnnouncement = false)
    {
        ...
        // 关键修复：必须在 StartCoroutine 之前立即设置 isToastPlaying = true，
        // 否则同帧内多次 EnqueueToast 会在协程体尚未执行时重复启动多个 ProcessToastQueue 协程，
        // 导致多个 Toast 并发显示、互相重叠。
        if (!isToastPlaying)
        {
            isToastPlaying = true;
    ...
    /// <summary>
    /// 立即隐藏当前 Toast 提示，并清空待显示的 Toast 队列。
    /// 供外部 UI（卖出模式、升级模式等）独占显示时调用，避免队列中的后续 Toast
    /// 在外部 UI 仍然显示时冒出来造成重叠。
    /// </summary>
    public void HideInfoToast()
    {
        // 清空待显示队列：外部已要独占显示，未显示的 Toast 不再重要
        toastQueue.Clear();
        currentToastMessage = null;
    ...
    private void UpdateUpgradePanelInfo()
    {
        ...
        if (upgradeSelectedTile.currentBuildingData != null)
        {
            ...
            // 不再弹 Toast 显示功能描述，避免与升级面板互相遮挡；改为日志输出
            UnityEngine.Debug.Log($"当前功能: {functionDesc}");
        ...
        if (nextBuilding != null)
        {
            if (!upgradeSelectedTile.CheckScaleForUpgrade(nextBuilding.requiredScale))
            {
                if (upgradeButton != null)
                {
                    ...
                    // 不再弹 Toast，改为在升级费用文本上追加地块等级提示
                    if (upgradeCostText != null)
                    {
                        upgradeCostText.text = $"需要地块等级: {(int)nextBuilding.requiredScale}";
                        upgradeCostText.color = Color.red;
                    }
                }
            }
