...
public class GameManager : MonoBehaviour
{
    ...
    void InitializeMusicSystem()
    {
        ...
        {
            Debug.LogWarning("MusicManager ??ß·??????");
        }
    }
    ...
    void GiveStartingItemsToPlayers()
    {
        ...
        {
            Debug.LogWarning("ItemManager ¶ƒ?????????????????");
            return;
        }
    ...
    void InitializeSFXSystem()
    {
        ...
        if (SFXManager.Instance == null)
        {
            ...
            else
            {
                ...
                {
                    Debug.LogWarning("SFXConfig ¶ƒ??Inspector??Resources??????");
                }
            }
    ...
    private void ProcessPlayerEndTurnEffects(Player player)
    {
        ...
        {
            Debug.Log($"{player.playerName} ???????ßπ??: {incomeReduction * 100}%");
        }

        float taxReduction = player.GetTaxReduction();
        if (taxReduction > 0)
        {
            Debug.Log($"{player.playerName} ??????ßπ??: {taxReduction * 100}%");
        }
    }
    ...
    public void UpdateUI()
    {
        ...
        if (currentTileText != null && currentPlayer.currentTile != null)
            currentTileText.text = $"???¶À??: {currentPlayer.currentTile.tileName}";
    ...
    void DebugGameState()
    {
        ...
        if (currentPlayer != null)
        {
            ...
            Debug.Log($"¶À??: {currentPlayer.currentTile.tileName}");
        ...
        if (boardManager != null)
        {
            ...
            foreach (BoardTile tile in boardManager.allTiles)
            {
                if (tile.tileType == BoardTile.TileType.Start)
                {
                    ...
                    if (tile.currentBuilding != null)
                    {
                        Debug.LogError($"????: {tile.tileName} ?ßﬂ???");
                    }
                }
            }
        }
