...
public class GameManager : MonoBehaviour
{
    ...
    private int nextPressureAt = 1;          // ??¦Ä????????????
    ...
    void InitializeMusicSystem()
    {
        ...
        {
            Debug.LogWarning("MusicManager ??§á??????????");
        }
    }
    ...
    void InitializeSFXSystem()
    {
        ...
        if (SFXManager.Instance == null)
        {
            ...
            else
            {
                ...
                {
                    Debug.LogWarning("SFXConfig ¦Ä?????????Inspector??????????Resources?????");
                }
            }
    ...
    void FindAllPlayers()
    {
        ...
        if (players.Count == 0)
        {
            Debug.LogWarning("¦Ä????¦Ê?Player???????÷ ???§Ö?Player?????");
        }
    }
    ...
    void SetupButtonEvents()
    {
        ...
        {
            Debug.LogWarning("RollDiceButton ¦Ä??Inspector??????????????????");
    ...
    void HandleJailTurn()
    {
        ...
        else
        {
            ...
            if (uiManager != null)
            {
                uiManager.ShowToast($"{currentPlayer.playerName} ?????§Ý???{currentPlayer.jailTurnsRemaining}???", 2f);
            }
    ...
    public void UpdateUI()
    {
        ...
        if (currentTileText != null && currentPlayer.currentTile != null)
            currentTileText.text = $"¦Ë??: {currentPlayer.currentTile.tileName}";
    ...
    void DebugGameState()
    {
        ...
        if (currentPlayer != null)
        {
            ...
            Debug.Log($"???¦Ë??: {currentPlayer.currentTile?.tileName}");
        ...
        if (boardManager != null)
        {
            ...
            foreach (BoardTile tile in boardManager.allTiles)
            {
                if (tile.tileType == BoardTile.TileType.Start)
                {
                    ...
                    {
                        Debug.LogError($"????: {tile.tileName} ???§ß???");
                    }
                }
            }
        }
