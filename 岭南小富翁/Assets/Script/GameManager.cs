...
public class GameManager : MonoBehaviour
{
    ...
    private void ApplyBankruptDebuff(Player player)
    {
        ...
        if (BuffSystem.Instance != null)
        {
            ...
            string sourceName = "?????";
            string description = $"???曳??????{bankruptGraceRounds}?????????????????????";
        ...
        if (UIManager.Instance != null)
        {
            ...
            {
                toastMessage = $"{player.playerName} ???????曳????";
            }
    ...
    public void UpdateUI()
    {
        ...
        if (currentTileText != null && currentPlayer.currentTile != null)
            currentTileText.text = $"竹??: {currentPlayer.currentTile.tileName}";
